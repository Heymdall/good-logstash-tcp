...
class GoodLogstashTcp extends Stream.Writable {
    constructor(config) {
        ...
        this._logstash = new Logstash(this._config.tlsOptions, this._config.messageDecorator);
        this._logstash.on('error', function (error) {
            console.error('good-logstash-tcp error', error);
        });
    }
